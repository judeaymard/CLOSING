    ...
    <div className="space-y-8 animate-fade-in-up">
      <div>
        ...
        <p className="text-xs text-slate-400 mt-1">
          Gestion des encaissements terrain (Cash on Delivery), commissions prélevées (2 800 F / colis) et reversements MoMo aux boutiques.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-[#090e22] border border-slate-800 rounded-3xl p-6 shadow-xl space-y-3">
          ...
          <p className="text-[11px] text-slate-500">{totalDeliveredOrders.length} commandes livrées & encaissées</p>
        </div>
        ...
        <div className="bg-[#090e22] border border-cyan-900/40 rounded-3xl p-6 shadow-xl space-y-3">
          ...
          <p className="text-[11px] text-slate-500">800 F (Closing) + 2 000 F (Livraison) / colis</p>
        </div>
        ...
        <div className="bg-[#090e22] border border-emerald-900/40 rounded-3xl p-6 shadow-xl space-y-3">
          ...
          <p className="text-[11px] text-slate-500">Montant total à transférer par Mobile Money</p>
        </div>
      </div>

      {/* REVERSEMENTS PAR PARTENAIRE TABLE */}
      <div className="bg-[#090e22] border border-slate-800 rounded-3xl shadow-xl overflow-hidden">
        <div className="p-6 border-b border-slate-800">
          <h3 className="text-base font-bold text-white">Soldes & Reversements par Boutique</h3>
          <p className="text-xs text-slate-400 mt-0.5">Calcul automatique des parts e-commerçants avec accès direct à leur bilan</p>
        </div>

        <div className="overflow-x-auto dark-scrollbar">
          <table className="w-full text-left text-xs whitespace-nowrap">
            <thead className="bg-slate-900 border-b border-slate-800 text-slate-400 font-extrabold uppercase tracking-wider text-[10px]">
              <tr>
            ...
            <tbody className="divide-y divide-slate-800/60 font-medium">
              {activePartners.map((ptn) => {
                ...
                return (
                  <tr key={ptn.id} className="hover:bg-slate-900/50 transition-colors">
                    <td className="py-4 px-4 font-black text-white">
                      <Link
                        href={`/admin/partenaires/${ptn.id}`}
                        className="hover:text-[#06b6d4] flex items-center gap-1.5"
                      >
                        <Store className="w-3.5 h-3.5 text-[#06b6d4]" />
                        <span>{ptn.companyName}</span>
                      </Link>
                    </td>
                    <td className="py-4 px-4 text-slate-300">{ptn.fullName}</td>
                    ...
                    <td className="py-4 px-4 font-bold text-[#06b6d4]">-{formatCFA(comm)}</td>
