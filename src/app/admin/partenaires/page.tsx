...
export default function AdminPartenairesPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      ...
      {/* 🧭 NAVIGATION TABS (ACTIFS VS EN ATTENTE) */}
      ...
      {activeTab === "attente" && (
        <div className="space-y-4">
          <div className="p-4 rounded-2xl bg-amber-950/40 border border-amber-500/30 text-amber-300 text-xs flex items-start gap-3">
            ...
            <div>
              <p className="font-bold text-amber-200">
                Processus de validation d&apos;inscription scmslivraison
              </p>
          ...
          {displayedPartners.length === 0 ? (
            <div className="py-16 text-center text-slate-500 text-xs bg-[#090e22] border border-slate-800 rounded-3xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {displayedPartners.map((ptn) => (
                <div
                  key={ptn.id}
                  className="bg-[#090e22] border-2 border-amber-500/30 hover:border-amber-500/60 rounded-3xl p-6 shadow-xl space-y-5 flex flex-col justify-between transition-all"
                >
                  <div className="space-y-3">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-300 font-semibold">{ptn.fullName}</p>
                        </div>
                      </div>
                    ...
                    {ptn.notes && (
                      <div className="p-3 rounded-xl bg-slate-900/90 border border-slate-800 text-[11px] text-slate-400 space-y-0.5">
                        <span className="text-[#06b6d4] font-bold">Demande du commerçant :</span>
                        <p>{ptn.notes}</p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="pt-4 border-t border-slate-800 flex flex-col sm:flex-row gap-2.5">
                    <a
                      href={`tel:${ptn.phone}`}
                      className="px-4 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-white text-xs font-bold flex items-center justify-center gap-1.5"
                    >
                    ...
                    <button
                      ...
                      className="flex-1 py-2.5 px-4 rounded-xl bg-[#06b6d4] hover:bg-cyan-600 text-white text-xs font-black shadow-lg shadow-cyan-500/20 transition-all flex items-center justify-center gap-2"
                    >
          ...
          {displayedPartners.map((ptn) => {
            ...
            return (
              <div
                ...
                className="bg-[#090e22] border-2 border-slate-800 hover:border-[#06b6d4] rounded-3xl p-6 shadow-xl space-y-6 flex flex-col justify-between transition-all cursor-pointer group"
              >
                <div className="space-y-4">
                  {/* Header */}
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 rounded-2xl bg-[#06b6d4] text-white flex items-center justify-center font-black text-lg shadow-md shadow-cyan-500/20 group-hover:scale-105 transition-transform">
                        {ptn.companyName.charAt(0)}
                      </div>
                      <div>
                        <h3 className="text-lg font-black text-white group-hover:text-[#06b6d4] transition-colors">
                          {ptn.companyName}
                        </h3>
                        <p className="text-xs text-slate-400 font-semibold">{ptn.fullName}</p>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-3 gap-2.5 pt-2 text-center">
                    <div className="bg-slate-900/90 p-3 rounded-2xl border border-slate-800">
                      <p className="text-[10px] text-slate-500 font-bold uppercase">Produits</p>
                      <p className="text-base font-black text-[#06b6d4] mt-0.5">
                        {ptnProducts.length}
                      </p>
                    </div>

                    <div className="bg-slate-900/90 p-3 rounded-2xl border border-slate-800">
                      <p className="text-[10px] text-slate-500 font-bold uppercase">Commandes</p>
                      <p className="text-base font-black text-white mt-0.5">{ptnOrders.length}</p>
                    </div>

                    <div className="bg-slate-900/90 p-3 rounded-2xl border border-slate-800">
                      <p className="text-[10px] text-slate-500 font-bold uppercase">Livrées</p>
                ...
                <div className="pt-4 border-t border-slate-800 flex items-center justify-between gap-2">
                  <button
                    ...
                    className="px-3.5 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 text-xs font-bold text-slate-300 hover:text-white border border-slate-800 flex items-center gap-1.5"
                  >
      ...
      {partnerToApprove && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#090e22] border border-slate-800 w-full max-w-xl rounded-3xl p-6 sm:p-8 space-y-6 shadow-2xl relative max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between pb-4 border-b border-slate-800">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-2xl bg-[#06b6d4] text-white flex items-center justify-center font-black text-lg">
            ...
            <form onSubmit={handleConfirmValidation} className="space-y-5">
              <div className="space-y-1">
                <h4 className="text-xs font-black text-[#06b6d4] uppercase tracking-wider flex items-center gap-1.5">
                  <Package className="w-4 h-4" /> Produits confiés par le commerçant
                </h4>
                <p className="text-[11px] text-slate-400">
                  Renseignez le(s) produit(s) physique(s) et la quantité initiale déposée dans l&apos;entrepôt. Vous pourrez en ajouter d&apos;autres à tout moment.
                </p>
              </div>
              ...
              <div className="space-y-3">
                {productRows.map((row, index) => (
                  <div
                    key={index}
                    className="p-3.5 rounded-2xl bg-slate-900/90 border border-slate-800 space-y-2.5 relative"
                  >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                      <div className="sm:col-span-1">
                        ...
                        <input
                          ...
                          className="w-full px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-xs text-white focus:outline-none focus:border-[#06b6d4]"
                        />
                      </div>
                ...
                <button
                  ...
                  className="w-full py-2.5 rounded-xl border border-dashed border-slate-700 hover:border-[#06b6d4] text-[#06b6d4] text-xs font-bold flex items-center justify-center gap-1.5 transition-colors"
                >
              ...
              <div className="pt-4 border-t border-slate-800 flex items-center justify-end gap-3">
                ...
                <button
                  type="submit"
                  className="px-6 py-2.5 rounded-xl bg-[#06b6d4] hover:bg-cyan-600 text-white text-xs font-black shadow-lg shadow-cyan-500/25 flex items-center gap-2"
                >
